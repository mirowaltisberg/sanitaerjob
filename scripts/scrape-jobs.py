"""Scrape a fresh chunk of genuine Swiss sanitary jobs into JSON.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_sanitaer_title
...
def is_relevant_job(job: dict) -> bool:
    """Return True only for clearly sanitary, Swiss, German-language roles."""
    ...
    return classify_sanitaer_title(title)["disposition"] == "ACCEPT"
...
def _scrape_jobs_worker(result_queue, scrape_kwargs: dict) -> None:
    ...
    # JobSpy can log source request URLs (including the query/location) even
    # with verbose=0. Keep worker output fully aggregate-only for CI logs.
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Swiss sanitary jobs")
