"""Validate, merge and publish one complete sanitary job snapshot.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_sanitaer_title, validate_raw_job_identity
...
def is_recent_real_job(job: Any, cutoff: date, today: date) -> bool:
    ...
    if validate_raw_job_identity(job) is not None:
        return False
    if classify_sanitaer_title(title)["disposition"] != "ACCEPT":
        return False
...
def fetch_existing_jobs(client: Client) -> dict[str, date | None]:
    ...
    while True:
        ...
        for row in rows:
            ...
            if job_id.startswith(ID_PREFIX):
...
def delete_stale_jobs(client: Client, stale_ids: set[str]) -> None:
    safe_ids = sorted(job_id for job_id in stale_ids if job_id.startswith(ID_PREFIX))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish one validated sanitary job snapshot")
    ...
    try:
        ...
        if args.plan:
            ...
            print(
                ...
                f"{len(existing)} current-prefix rows, "
                f"{existing_trade_count - len(existing)} legacy/invalid-ID rows, "
                f"{len(jobs)} validated candidates, "
                f"{len(retained_ids)} retained IDs, "
                f"{len(set(existing) - fresh_ids)} current-prefix rows eligible for scoped prune."
            )
