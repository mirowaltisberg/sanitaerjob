"""Shared deny-first Sanitär classifier backed by src/config/job-safety.json."""
...
def classify_sanitaer_title(title_value: Any) -> dict[str, Any]:
    ...
    if not match:
        return {"disposition": "REJECT", "reason": "no-sanitaer-title-signal"}
    ...
    return {
        "disposition": "ACCEPT",
        "reason": "specific-sanitaer-title" if specific else "broad-role-with-sanitaer-context",
