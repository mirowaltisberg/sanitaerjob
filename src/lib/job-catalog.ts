...
import { classifySanitaerTitle, isValidSanitaerJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifySanitaerTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
function matchesQuery(job: JobListing, query: string): boolean {
  ...
  const hasPublicMatch = (queryToken: string) => {
    if (queryToken === "sanitär" || queryToken === "sanitaer") {
      return true;
    }
...
export async function getJobListingById(input: {
  ...
  if (!isValidSanitaerJobId(input.id)) {
    return null;
  }
