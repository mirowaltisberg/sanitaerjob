...
export function classifySanitaerTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-sanitaer-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-sanitaer-title" : "broad-role-with-sanitaer-context",
...
export function isValidSanitaerJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getSanitaerProfile(profile: JobSafetyProfile) {
