...
import { classifySanitaerTitle, getSanitaerProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifySanitaerTitle(input.title);
  if (
  ...
  const profile = getSanitaerProfile(classification.profile);
