...
const homepageMetadata: Metadata = {
  ...
  description:
    "Finde Stellenangebote für Sanitärinstallateur EFZ, Sanitärmonteur, Servicetechnik, Planung und Projektleitung Sanitär in der Schweiz.",
              ...
              <details className="group relative hidden sm:block">
                ...
                <ul className="search-dropdown absolute right-0 top-[calc(100%+8px)] z-50 min-w-56 overflow-hidden border bg-white py-1 shadow-lg">
                  {EMPLOYER_MENU_ITEMS.map((item) => (
                    <li key={item.href}>
                      <Link href={item.href} className="search-dropdown-item block px-4 py-2.5 text-sm text-slate-700 outline-none focus:bg-primary/10 focus:text-slate-900">
                        {item.label}
                      </Link>
                    </li>
                  ))}
          ...
          <section className={`trade-hero relative z-20 overflow-visible ${hasActiveSearch ? "pb-5 pt-8 sm:pb-7 sm:pt-10 md:pt-12" : "pb-6 pt-10 sm:pb-9 sm:pt-14 md:pt-20"}`}>
            <div className="container mx-auto max-w-6xl px-4 sm:px-6">
              <div className="trade-hero-grid">
                <div>
                  <p className="trade-kicker">Sanitär · ganze Schweiz</p>
                  <h1 className="trade-display">
                    Sanitärjobs.
                    <em>Präzise gefunden.</em>
                  </h1>
                  <p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-lg">
                    Stelleninserate für Sanitärinstallation, Montage, Planung, Service und Projektleitung. Kompakt filtern nach Beruf, Ort und Pensum.
                  </p>
                </div>
                <div className="pipe-schematic" aria-hidden="true">
                  <span className="pipe-schematic__label">Leitung SAN / Suche CH</span>
                </div>
              </div>
              ...
              <form action="/" method="get" className="relative z-30">
                <div className="search-bench min-w-0">
                  <div className="search-field relative flex-1">
                    ...
                    <datalist id="homepage-job-suggestions">
                      {JOB_SUGGESTIONS.map((suggestion) => <option key={suggestion} value={suggestion} />)}
                    </datalist>
                  </div>
                  <div className="search-field relative flex-1">
                    ...
                    <datalist id="homepage-location-suggestions">
                      {LOCATION_SUGGESTIONS.map((suggestion) => <option key={suggestion} value={suggestion} />)}
                    </datalist>
                  </div>
                  <button type="submit" className="btn-interactive inline-flex h-12 w-full items-center justify-center bg-primary px-6 text-base font-bold text-primary-foreground hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 md:w-auto sm:px-8">
                    Jobs suchen
                  </button>
                </div>
              </form>
